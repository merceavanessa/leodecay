...
class OMNIPreprocessor():
    ...
    def calculate_position_anomalies(self, df, id_type="IMF", gradient_thr=0.5):
        ...
        df['L1 Distance Gradient Residuals'] = df.groupby(id_column)['L1 Distance Gradient'].transform(
        ...
        df['Position Anomaly'] = df.groupby(id_column)['Position Anomaly'].transform(
            lambda v: v.rolling(window=3, center=True).max())
        df['Position Anomaly'] = df.groupby(id_column)['Position Anomaly'].transform(
            lambda v: v.interpolate(method='nearest')).ffill().bfill()
        ...
        df[position_columns] = df.groupby(id_column)[position_columns].transform(
            lambda v: v.interpolate(method='nearest'))
    ...
    def parallel_slope_calculation(self, dataframe, window_size, cadence, day_interval, columns, num_processes=8,
                                   centered=True):
        ...
        with mp.Pool(processes=num_processes) as pool:
            processed_chunks = pool.starmap(self.calculate_slopes if centered else self.calculate_slopes_noncentered,
                                            [(chunk, window_size, cadence, day_interval, columns) for chunk in chunks])
...
class PODPreprocessor():
    ...
    def preprocess_pod(self, lst_path, out_path, cadence='30s'):
        ...
        for satellite_name in satellite_names:
            ...
            logger.info(
                f"Finished Computing Signal Decomposition for {satellite_name} at time {timeit.default_timer()}.")
    ...
    def parallel_slope_calculation(self, dataframe, window_size, cadence, day_interval, num_processes=8, centered=True):
        ...
        if len(chunks) > 1:
            ...
            with mp.Pool(processes=num_processes) as pool:
                processed_chunks = pool.starmap(self.calculate_slope if centered else self.calculate_slope_noncentered,
                                                [(chunk, window_size, cadence, day_interval) for chunk in chunks])
            final_df = pd.concat(processed_chunks)
    ...
    def add_od_features(self, dataframe, centered=True):
        ...
        dataframe.loc[::window_size * 24 + 1, 'daily_max_T+R'] = dataframe['unresolved'].rolling(
            window=window_size * 24,
...
class SMOSPreprocessor:
    def process_smos_file(self, file_path):
        """Process SMOS solar flux data from fixed-width format files.

        Removes footer section and extracts timestamp and sun flux using fixed-width parsing.
        """
        try:
            logger = logging.getLogger(__name__)
            ...
            marker = 'List of SMOS L1B input product'
            ...
            df = pd.read_fwf(
                ...
                header=None,
                colspecs=[(0, 29), (45, 61)],
                names=['time', 'sun_flux_smos']
            )

            if df.empty:
                logger.warning(f"SMOS file {file_path} is empty after parsing.")
                return None
            ...
            logger.info(
                f"Successfully parsed SMOS file {file_path}: {len(df)} records from {df.index.min()} to {df.index.max()}")
            return df

        except Exception as e:
            logger = logging.getLogger(__name__)
            logger.error(f"Failed to parse SMOS file {file_path}: {e}")
            import traceback
    ...
    def preprocess_smos(self, input_path: str, output_path: str, cadence='30s', interpolate=False):
        ...
        with ProcessPoolExecutor() as executor:
            dataframes = list(executor.map(self.process_smos_file, smos_files))
...
class DataImputer(BaseEstimator, TransformerMixin):
    def __init__(self, strategy='mean', columns=None, all_cols=False, fill_value=None, excluded_columns=None):
        ...
        if strategy == 'constant':
            if str(fill_value).lower() in ('none', 'null', '~', '', 'nan'):
                fill_value = np.nan
    ...
    def fit(self, X, y=None):
        ...
        if self.all_cols:
            # Exclude the columns that were already imputed in specific step
            cols_to_impute = [col for col in columns_with_na if col not in self.excluded_columns]
            print(self.strategy)
            print(cols_to_impute)
            print(self.excluded_columns)
        else:
            cols_to_impute = self.columns

        if len(cols_to_impute) > 0:
            if self.imputer != None:
                self.imputer.fit(X[cols_to_impute])
        else:
            print('No columns to impute. Skipping fit. ')
...
class PipelinesProcessor:
    ...
    def preprocess(self):
        ...
        pipe.append(("Drop columns.", ColumnDropper(columns_to_drop=self.pipeline_configuration['columns_to_drop'],
        ...
        specific_imputed_columns = []  # Track which columns were imputed in step 5
        for scheme in self.pipeline_configuration['imputing_schemes']['specific']:
            ...
            if len(columns) > 0:
                fill_value = self.pipeline_configuration['imputing_schemes']['specific'][scheme].get(
                    'value', None)
                specific_imputed_columns.extend(columns)
                pipe.append((f"Imput missing values (specific columns) with scheme {scheme}.",
                             DataImputer(strategy=scheme,
                                         columns=columns,
                                         fill_value=fill_value)))

        # 6. input rest columns (excluding the ones already imputed in step 5)
        pipe.append(
